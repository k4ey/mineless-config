...
local function noiser(self, args)
  local leftReach = args and args.leftReach or 50
  local pitch = args and args.pitch or 50

  local toEdge = plotInMovement(getAccelerationVector(35), nil, leftReach)

  coroutine.yield()
  hud3D.clearAll()
  if toEdge == 0 then return end
  if not chance(leftReach - toEdge, leftReach) then
    return
  end
  local yaw, oldPitch = getRotation()
  log((leftReach - toEdge))
  local newYaw = yaw + (leftReach - toEdge) * 1.5
  asyncLook(yaw, newYaw, oldPitch, pitch, 1000)
end
